  ...
  return (
    <>
      ...
      <form className="card row" onSubmit={create}>
        ...
        <button className="primary">Add policy</button>
          ...
          <tbody>
            {policies.data?.map((p) => (
              <tr key={p.id}>
                ...
                <td><button onClick={() => toggle(p)}>{p.enabled ? 'Disable' : 'Enable'}</button> <button className="danger" onClick={() => remove(p)}>Delete</button></td>
              </tr>
            ))}

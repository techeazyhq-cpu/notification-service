...
The admin UI signs in with a user name and password (first start: `admin` / `admin`, seeded from `ADMIN_USERNAME` / `ADMIN_PASSWORD` only when there is no administrator yet, and always created with the `ADMIN` role). Open **My account** to change the password (12+ characters) and to turn on two-factor authentication: scan the QR code with an authenticator app, enter the 6-digit code, and keep the 10 recovery codes. Sessions end after 30 idle minutes; 5 wrong attempts lock the account for 15 minutes. Set `ADMIN_TWO_FACTOR_KEY` to your own secret (it encrypts the stored two-factor keys). API scripts sign in with `POST /api/admin/auth/login` and send the returned token as `Authorization: Bearer ...`; `scripts/seed.mjs` does this and reads `ADMIN_OTP` when two-factor is on. See ADR-005.

Administrators have one of three roles — `VIEWER` (read-only), `OPERATOR` (adds day-to-day recovery actions: retry a message, reprocess dead letters, handle an erasure request, top up prepaid credit) or `ADMIN` (full access, including managing other administrators). Manage them at `/api/admin/administrators` (`GET` to list, `POST` to create, `PUT /{id}/role` to change a role, `DELETE /{id}` to remove one — always `ADMIN`-only; there is no admin-ui page for this yet). The last remaining `ADMIN` can't be demoted or deleted, and nobody can delete their own account. See ADR-015.
